...
class Linear:
    ...
    def forward(self, A):
        """
        :param A: Input to the linear layer with shape (*batch_dims, in_features)
        :return: Output Z with shape (*batch_dims, out_features)
        ...
        # Reshape input to 2D: (batch_size, in_features) where batch_size = prod(batch_dims)
    ...
    def backward(self, dLdZ):
        """
        :param dLdZ: Gradient of loss wrt output Z (*batch_dims, out_features)
        :return: Gradient of loss wrt input A (*batch_dims, in_features)
        """
